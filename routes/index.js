...
router.post('/dat-hang.html',checkLogin, function (req, res) {
...
router.get('/dat-hang.html',checkLogin, function (req, res) {
	var giohang = new GioHang( (req.session.cart) ? req.session.cart : {items: {}} );
	//var data = giohang.convertArray();
...
router.get('/gio-hang.html', function (req, res) {
	...
	var data = giohang.convertArray();
	Cate.find().then(function(cate){
		res.render('site/page/cart', {data: data, cate:cate});
	})
