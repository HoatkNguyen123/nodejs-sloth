...
var upload = multer({ storage, fileFilter, limits: { fileSize: 102400 } });
...
router.post('/them-product.html', checkAdmin, function (req, res) {
	
	upload.single('hinh')(req, res, err => {
	
		if (err) {
			req.flash('error_msg', err.message);
			return res.redirect('/admin/product/them-product.html');
		}
